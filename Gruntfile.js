module.exports = function(grunt) {
  ...
  grunt.initConfig({
    ...
    copy: {
      build: {
        files: [
          {expand: true, flatten: true, dest: '<%= deployFolder %>', src: ['build/game.min.css', 'build/game.min.js', 'build/index.html', 'img/**', 'update.json']},
        ]
      },
      debug: {
        files: [
          {dest: '<%= debugFolder %>/index.html', src: ['build/index.debug.html']},
          {expand: true, flatten: true, dest: '<%= debugFolder %>', src: ['build/game.css', 'build/game.js', 'img/**', 'update.json']},
        ]
      }
    },
